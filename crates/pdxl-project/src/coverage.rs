//! Schema-coverage survey: which game directories a schema does *not*
//! model yet, ranked as modelling targets. Game-agnostic — the survey roots
//! and schema come from the rules crate (through the `pdxl-game` facade in
//! the `schema-gaps` bin).
//!
//! Walks the definition roots of a game installation and for every leaf directory holding `.txt` script files
...
/// Surveys `game_root` against `schema`. `roots` are the game's definition
/// roots (CK3: `common`/`events`/`history`; EU5 carries its module prefixes,
/// `in_game/common` etc.); `context_roots` are the structural-context
/// prefixes (documented bodies count as covered). Returns one report per
/// leaf directory containing `.txt` files, unsorted — rank with
/// [`DirReport::score`].
pub fn survey(
    game_root: &Path,
    schema: &Schema,
